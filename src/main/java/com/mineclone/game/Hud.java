...
public class Hud {
    ...
    private static final float[] VIDEO_RANGE_MIN  = { 2f,  50f, 0f, 30f,  0f, 0f };
    private static final float[] VIDEO_RANGE_MAX  = { 16f, 120f, 2f, 260f, 3f, 2f };
    private static final String[] GUI_SCALE_LABELS = { "Auto", "Small", "Normal", "Large" };
    /** Fast — без теней и пост-эффектов; Fancy — тени + bloom; Ultra — мягкий PCF. */
    private static final String[] SHADER_LABELS = { "Fast", "Fancy", "Ultra" };
    private static final int VIDEO_SLIDERS = 6;
    ...
    /**
     * @param values  in/out: [renderDist, fov, brightness, maxFps(30-260; &ge;255=unlimited),
     *                guiScale(0-3), shaderQuality(0-2)]
    ...
    public MenuAction drawVideoSettings(int sw, int sh, double mx, double my,
        ...
        float pw = 480f, ph = 470f + ROW_STEP;
        float px = sw / 2f - pw / 2f, py = sh / 2f - ph / 2f;
        float slW = pw - 60f, slX = px + 30f;

        float[] slY = new float[VIDEO_SLIDERS];
        for (int i = 0; i < VIDEO_SLIDERS; i++) slY[i] = py + 74f + i * ROW_STEP;

        updateSliders(values, VIDEO_RANGE_MIN, VIDEO_RANGE_MAX, slX, slW, slY,
                VIDEO_SLIDERS, mx, my, mouseDown);
        // Clamp the stepped sliders to integer positions
        values[4] = Math.round(values[4]);
        values[5] = Math.round(values[5]);

        float tgW = (slW - 10f) / 2f, tgH = 40f;
        float tg0Y = slY[VIDEO_SLIDERS - 1] + SL_H + 36f;
        float tg1Y = tg0Y + tgH + 10f;

        ui.begin(sw, sh);
        drawPanelBg(sw, sh, px, py, pw, ph);
        drawSliderTracks(slX, slW, slY, values, VIDEO_RANGE_MIN, VIDEO_RANGE_MAX,
                VIDEO_SLIDERS, mx, my);
        ...
        int sqIdx = Math.max(0, Math.min(2, Math.round(values[5])));
