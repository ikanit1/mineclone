...
/** Original deterministic core checks, moved without changing their assertions. */
final class CoreTests {
    private CoreTests() {}

    static void runAll(TestMain.Runner r) {
        r.run("a stale mesh never overwrites a fresher one", CoreTests::testMeshVersionRejectsStale);
        r.run("the emitter list tracks the blocks it describes", CoreTests::testEmitterListMatchesChunk);
        r.run("player edits jump the mesh queue ahead of distance", CoreTests::testMeshPriorityOrder);
        r.run("frame profiler keeps the worst frame, not the average", CoreTests::testFrameProfilerWorst);
        r.run("removing the last torch actually puts the light out", CoreTests::testLastTorchGoesOut);
        r.run("World.key round-trips through (cx,cz)", CoreTests::testWorldKeyRoundTrip);
        r.run("BlockType.byId guards out-of-range ids", CoreTests::testByIdGuard);
        r.run("ItemStack clamps and stacks", CoreTests::testItemStack);
        r.run("Inventory add merges then fills", CoreTests::testInventoryAdd);
        r.run("Inventory removeOne empties slot", CoreTests::testInventoryRemoveOne);
        r.run("Inventory left/right click stack ops", CoreTests::testInventoryClick);
        r.run("tools never stack and keep their own wear", CoreTests::testToolStacks);
        r.run("tool tier gates the drop, class gates the speed", CoreTests::testToolGating);
        r.run("tools wear out and vanish", CoreTests::testToolWear);
        r.run("crafting consumes exactly what the recipe says", CoreTests::testCrafting);
        r.run("every recipe is reachable and unambiguous", CoreTests::testRecipeTable);
        r.run("food stacks by kind and never with blocks", CoreTests::testFoodStacks);
        r.run("hunger drains, gates regen and stops short of killing",
                CoreTests::testHunger);
        r.run("peaceful mobs drop meat, zombies drop nothing", CoreTests::testMobDrops);
        r.run("new biome blocks registered", CoreTests::testBiomeBlocks);
        r.run("Mob floats upward in water", CoreTests::testMobBuoyancy);
        r.run("Water drains, weakens and falls without sideways arms", CoreTests::testWaterFlow);
        r.run("Single water source cannot flood a stepped hillside", CoreTests::testWaterSlope);
        r.run("Water prefers nearest downhill outlet", CoreTests::testWaterOutlet);
        r.run("Weather cycle follows biome precipitation rules", CoreTests::testWeather);
        r.run("Flight stop clears previous fall damage", CoreTests::testFlightStopsFall);
        r.run("Mob animation moves at rest and settles after walking", CoreTests::testMobAnimation);
        r.run("sun light direction never grazes the horizon", CoreTests::testSunLightDirection);
        r.run("shadow strength fades across sunrise", CoreTests::testShadowStrength);
        r.run("shadow cascade covers its radius and snaps to texels",
                CoreTests::testShadowCascade);
        r.run("lazy cascade rebuilds on age, camera turn and time jump",
                CoreTests::testCascadeStaleness);
        r.run("caves carve a sane share of the underground", CoreTests::testCavesCarveUnderground);
        r.run("caves keep bedrock and the seabed intact", CoreTests::testCavesKeepSeabedAndBedrock);
        r.run("ores sit in their bands and in stone only", CoreTests::testOreBandsAndHost);
        r.run("world generation is deterministic per seed",
                CoreTests::testWorldGenerationDeterministic);
        r.run("hostile spawn rule follows light, not the clock",
                CoreTests::testHostileSpawnLightRule);
        r.run("structures appear, stay inside their chunk and repeat per seed",
                CoreTests::testStructures);
        r.run("grass spreads onto bare dirt and dies when covered",
                CoreTests::testGrassTick);
        r.run("orphaned leaves decay, supported ones stay", CoreTests::testLeafDecay);
        r.run("cactus grows up to its limit", CoreTests::testCactusGrowth);
        r.run("setBlock skips relighting when opacity is unchanged",
                CoreTests::testSetBlockSkipsRelight);
        r.run("incremental sky light matches a full reflood",
                CoreTests::testIncrementalSkyLight);
        r.run("water and leaves attenuate skylight by material",
                CoreTests::testMaterialSkyAttenuation);
        r.run("fire spreads along fuel and eats it", CoreTests::testFireSpreadsAndConsumes);
        r.run("fire without fuel burns out", CoreTests::testFireDiesWithoutFuel);
        r.run("water and rain put fire out, a roof saves it",
                CoreTests::testWaterAndRainExtinguishFire);
        r.run("snow settles in the tundra, thickens and melts",
                CoreTests::testSnowAccumulation);
        r.run("player stops striding when he stops walking",
                CoreTests::testPlayerWalkAmplitude);
        r.run("player model faces the same direction as the camera",
                CoreTests::testPlayerModelFacing);
        r.run("hotbar selection spring starts and settles",
                CoreTests::testSelectSpring);
        r.run("compass heading turns the right way", CoreTests::testCompassHeading);
        r.run("compass clock agrees with the sun", CoreTests::testCompassClock);
        r.run("A* walks around a wall instead of into it", CoreTests::testPathAroundWall);
        r.run("A* climbs a step and drops off a ledge", CoreTests::testPathVertical);
        r.run("A* refuses the impossible and stays cheap", CoreTests::testPathLimits);
        r.run("chasing zombie walks around a wall to the doorway",
                CoreTests::testZombieNavigatesWall);
        r.run("zombies flank instead of queueing up", CoreTests::testZombieFlanking);
        r.run("scattered cows gather into a herd", CoreTests::testHerding);
        r.run("footprints hold, then fade out", CoreTests::testDecalFade);
        r.run("breaking a chest forgets what was inside", CoreTests::testChestLifecycle);
        r.run("chest slots follow the same click rules as the inventory",
                CoreTests::testContainerClicks);
        r.run("rivers are ribbons, not blotches", CoreTests::testRiverShape);
        r.run("rivers never cut canyons and never raise ground",
                CoreTests::testRiverCarveLimits);
        r.run("generated world actually has fresh water", CoreTests::testRiverInWorld);
        r.run("furnace smelts, burns fuel and stops when full",
                CoreTests::testFurnaceSmelting);
        r.run("cooking is worth the trouble", CoreTests::testCookedFoodBalance);
        r.run("sleep needs night and quiet, and wakes at dawn", CoreTests::testSleepRules);
        r.run("bedroll meshes half a block tall", CoreTests::testBedrollHeight);
        r.run("cave ambience is rare and only underground", CoreTests::testAmbientCave);
        r.run("water drowns out the cave, rain above never reaches it",
                CoreTests::testAmbientPriority);
        r.run("rain is heard only as far as the sky reaches", CoreTests::testRainNeedsSky);
        r.run("every ambient cue has files behind it", CoreTests::testAmbientAssets);
        r.run("a cave sounds more enclosed than a field", CoreTests::testAcousticProbe);
        r.run("spatial sound grows smoothly while approaching", CoreTests::testSpatialSoundGain);
        r.run("behaviour tree picks branches by priority", CoreTests::testBehaviorTree);
        r.run("burning zombie runs for shade when there is any",
                CoreTests::testZombieSeeksShelter);
        r.run("biome params sane", CoreTests::testBiomeParams);
        r.run("biome climate table", CoreTests::testBiomeClassify);
        r.run("biome provider deterministic", CoreTests::testBiomeDeterminism);
        r.run("biome provider covers all biomes", CoreTests::testBiomeCoverage);
        r.run("chunk surface matches biome", CoreTests::testChunkSurfaceMatchesBiome);
        r.run("biome borders have no cliffs", CoreTests::testHeightSmoothness);
        r.run("vegetation matches biome rules", CoreTests::testVegetationInvariants);
        r.run("sound materials for biome blocks", CoreTests::testBiomeSoundMaterials);
        r.run("BlockType drop table", CoreTests::testDropTable);
        r.run("MobType params sane", CoreTests::testMobTypeParams);
        r.run("EntityPhysics lands on ground", CoreTests::testEntityPhysicsFall);
        r.run("EntityPhysics stops at wall", CoreTests::testEntityPhysicsWall);
        r.run("EntityPhysics ray vs AABB", CoreTests::testRayAabb);
        r.run("Mob peaceful wanders and idles", CoreTests::testMobPeacefulStates);
        r.run("Mob flees when hurt", CoreTests::testMobFlee);
        r.run("Mob dies at zero health", CoreTests::testMobDeath);
        r.run("Zombie chases nearby player", CoreTests::testZombieChase);
        r.run("Zombie attack respects cooldown", CoreTests::testZombieAttackCooldown);
        r.run("Zombie is passive in creative", CoreTests::testZombieCreativePassive);
        r.run("Zombie burns under open sky", CoreTests::testZombieSunBurn);
        r.run("MobSpawner rules and caps", CoreTests::testMobSpawnerRules);
        r.run("MobSpawner needs loaded chunks", CoreTests::testMobSpawnerNeedsChunks);
        r.run("MobSpawner despawns distant mobs", CoreTests::testMobSpawnerDespawn);
        r.run("MobSkins generates distinct skins", CoreTests::testMobSkins);
        r.run("first-person hand stays on screen", CoreTests::testFirstPersonPoses);
        r.run("mob sounds resolve for every type", CoreTests::testMobSounds);
        r.run("Mob takes fall damage by impact speed", CoreTests::testMobFallDamage);
        r.run("Mob emits step sounds while walking", CoreTests::testMobStepSounds);
        r.run("EntityPhysics separates overlapping bodies", CoreTests::testSeparate);
        r.run("Mob sidesteps a wall instead of butting it", CoreTests::testMobSidestep);
        r.run("Mob ignores hits inside the invulnerability window", CoreTests::testMobInvulnWindow);
        r.run("Player ignores mob hits inside its window", CoreTests::testPlayerInvulnWindow);
        r.run("Zombie burns at /time set day, not only at noon", CoreTests::testZombieBurnsAtDay);
        r.run("Zombie needs line of sight to aggro", CoreTests::testZombieLineOfSight);
        r.run("Zombie has a blind spot behind it", CoreTests::testSightCone);
        r.run("walls muffle positional sound", CoreTests::testSoundOcclusion);
        r.run("darkness shortens the detection range", CoreTests::testSightRangeByLight);
        r.run("noise pulls a zombie to investigate, chase wins over noise",
                CoreTests::testHearingAndInvestigate);
        r.run("Dead mob lingers for the fall-over animation", CoreTests::testMobDeathLinger);
        r.run("Player regen pauses after a hit", CoreTests::testRegenPause);
    }

    static void runSaveTests(TestMain.Runner r) {
        r.run("level.dat carries tools with their wear", CoreTests::testToolSaveRoundTrip);
        r.run("level.dat carries food and hunger", CoreTests::testFoodSaveRoundTrip);
        r.run("level.dat save/load round-trip", CoreTests::testLevelRoundTrip);
        r.run("chunk save/load round-trip", CoreTests::testChunkRoundTrip);
        r.run("options.dat save/load round-trip", CoreTests::testOptionsRoundTrip);
        r.run("screen, graphics and gameplay options survive a save",
                CoreTests::testVideoOptionsRoundTrip);
        r.run("chest keeps its contents through a save", CoreTests::testChestSaveRoundTrip);
        r.run("furnace wastes no fuel and keeps state through a save",
                CoreTests::testFurnaceFuelAndSave);
        r.run("bedroll is craftable, layered and save-safe", CoreTests::testBedroll);
        r.run("atomic save leaves no .tmp files", CoreTests::testNoTempLeftovers);
        r.run("save overwrite keeps old data on rewrite", CoreTests::testOverwriteRoundTrip);
    }
    ...
    private static void testNoTempLeftovers() throws Exception {
        ...
        sm.flushAndAwait();
        assertTrue("level save completed before checking temporary files", new File(worldDir, "level.dat").isFile());
