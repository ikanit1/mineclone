...
### Предметы, инструменты и крафт
`ItemStack` держит **одно из трёх**: блок (`type`), инструмент (`tool`) или
еду (`food`) — ровно одно поле не null. Инструменты не стопкуются, у каждого свой износ
(`damage`), полоска прочности рисуется под иконкой.

`ToolType` — три класса (кирка, топор, лопата) на четырёх материалах. Класс
решает, что копается быстро (`BlockType.preferredTool`), уровень материала —
что вообще даёт дроп (`BlockType.requiredToolLevel`). Неподходящий инструмент
не ускоряет вообще: киркой по земле копается как руками.

`Recipes` — бесформенные рецепты, собираются из полки в экране инвентаря
...
требовательный рецепт обязан стоять раньше.

`FoodType` — мясо, падающее с мирных мобов (зомби не даёт ничего: иначе ночь
становится фермой). Сытость у игрока гейтит восстановление здоровья и на нуле
отнимает его, но не убивает — упирается в `STARVE_FLOOR`.

`CHEST` — контейнер на 27 слотов. Содержимое живёт **в чанке**
(`Chunk.getChest` / `World.getChest`), а не в блоке: в id блока помещается
один байт. Массив слотов заводится лениво, при первом открытии. Правила
клика общие с инвентарём — `Inventory.leftClick`/`rightClick` стали
статическими над произвольным массивом слотов, экземплярные делегируют им.
Сундук не ломается, пока содержимому некуда деться: предметных сущностей нет,
всё уходит прямо в инвентарь, а что не влезло — остаётся в мире вместе с
блоком. ADR: `knowledge/decisions/chests-and-containers.md`.

`FURNACE` — печь: три слота (что плавим / чем топим / что вышло), таблица
переплавки и топлива в `Smelting`, состояние в `Furnace` — чистая логика без
GL и без ссылки на мир, поэтому проверяется обычными тестами. Печи тикают
раз в `FURNACE_TICK` в радиусе `FURNACE_RADIUS` чанков вокруг игрока; чанк
помечается изменённым **только при смене слотов**, иначе горящая печь
переписывала бы свой файл четыре раза в секунду. Жареное мясо вдвое сытнее
сырого — ради этой разницы печь и строится.
...
возрождения. Правила — `SleepRules`, чистые функции без мира и без игрока.
Время **не заворачивается назад**: `nextDawn` прибавляет до ближайшего
восхода строго в будущем, иначе номер суток не растёт и сон возвращал бы
вчерашнюю фазу луны. После сна обязателен `invalidateShadows()` — каскады
перестраиваются лениво, и первый кадр нового дня иначе рисуется вчерашним
светом. Спальник — слой (`isLayered`), ставится с `meta = 3`, то есть ровно
половина блока. ADR: `knowledge/decisions/sleep-and-bedroll.md`.

Формат уровня поднят до v9: слот инвентаря размечен (пусто / блок /
инструмент / еда), добавлено поле сытости. Версии 1–8 читаются как раньше.
Формат настроек (`options.dat`) — v5: раскладка клавиш парами «имя действия —
код клавиши»; по имени, чтобы новое действие не сдвигало чужие клавиши. Версии
1–4 читаются с раскладкой по умолчанию.
`CHUNK_VERSION` поднят до 4 (2 — содержимое сундуков, 3 — состояние печей,
4 — предметы, лежащие в мире); младшие версии читаются как раньше — до этого
загрузчик отказывал на любой несовпавшей версии, то есть подъём версии
выбрасывал все правки во всех сохранённых мирах.
...
случайной тёмной точки рядом, а не в голове.
...
| `Hud` | `drawBlockIcon()` | Изометрический кубик в слоте: пропорции и яркость граней |
| `Hud` | `isCubeIcon()` | Что рисуется кубиком, а что остаётся плоским спрайтом |
...
| `ToolType` | таблица тиров | Класс, уровень, скорость и прочность двенадцати инструментов |
...
| `Recipes` | `ALL` | Таблица рецептов; порядок задаёт их вид на полке |
| `Hud` | `CRAFT_MAX / CRAFT_ROW_H` | Сколько рецептов помещается на полку (18 — вся таблица) |
...
| `FoodType` | `nutrition` | Сытость; у жареного вдвое выше сырого |
| `FoodType` | `nutrition` | Сколько сытости даёт кусок мяса |
...
| `MobType` | `drop() / dropCount()` | Что падает с моба |
