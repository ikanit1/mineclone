...
4. **Mobs** — `MobRenderer` рисует коробочные модели по скинам из `MobSkins`.
...
6. **HUD** — `Crosshair`, `Hud` (hotbar, menus, debug overlay via `Font`/`TextRenderer`/`UiRenderer`).
