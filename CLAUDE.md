...
- Экран — «Игра по сети» на титуле (`ui/MultiplayerScreen`): имя, транспорт,
  регион и список открытых комнат либо адрес хозяина, имя комнаты, выбор
  своего мира, свой ключ Photon мелкой строкой внизу. Настройки живут в
  `options.dat` (размеченный хвост v7, версия файла не поднималась).
