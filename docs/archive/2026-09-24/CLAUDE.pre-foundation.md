...
Historical snapshot; use ../../../CLAUDE.md for current guidance.
...
Tests run via `.\run-tests.ps1` (no JUnit — plain main + asserts). `TestMain` — the runner and the core suite; `FeatureTests`, `InventoryTests`, `WorldGenerationTests` and `NetworkTests` hang off it. Exit code is non-zero if any test fails.
...
**Замер рывков в живой игре:** `java -Dmineclone.stress=<секунды> -cp "out;libs/*" com.mineclone.Main` —
...
- **Край, а не биом**: одиннадцать биомов схлопываются в шесть краёв
  ...
  сборку, а не тихо замолкал. Одиннадцати треков не бывает; края розданы по
  тону семи существующих — растяжка до настоящих треков, но в пустыне уже
...
- Мир тикает `WorldSimulation` — **тем же классом, которым тикает игра**: два
  ...
  крутится вокруг участников, а не вокруг точки появления.
...
| `DedicatedServer` | `TICK_RATE / TIME_SCALE` | Темп мира сервера и темп его суток |
